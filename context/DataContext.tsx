...
export const DataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const deleteTransaction = async (id: string) => {
    ...
    if (account) {
      ...
      batch.update(doc(db, 'accounts', account.id), { balance: account.balance + amountDiff });
    }
    await batch.commit();
  };
    ...
    try {
      ...
      if (updates.length > 0) {
        ...
        stocks.forEach(stock => {
          ...
          if (update) batch.update(doc(db, 'stocks', stock.id), { currentPrice: update.price });
        });
        await batch.commit();
      }
    } finally {
  ...
  return (
    <DataContext.Provider value={{
      accounts, transactions, stocks, addAccount, deleteAccount, updateAccount,
      addTransaction, deleteTransaction, addStock, deleteStock, updateStockPrices,
      isLoadingStocks, currentUser, loadingData, configError, firestoreError
    }}>
      {children}
